					...
					<c:forEach var="schedule" items="${scheduleList}">
						<tr>
							<td><input type="text" name="title"
								value="${schedule.title}" form="form-${schedule.scheduleId}"></td>
							<td><input type="text" name="content"
								value="${schedule.content}" form="form-${schedule.scheduleId}"></td>
							<td><input type="date" name="startdate"
								value="${schedule.startdate}" class="startdate"
								form="form-${schedule.scheduleId}"></td>
							<td><input type="date" name="enddate"
								value="${schedule.enddate}" class="enddate"
								form="form-${schedule.scheduleId}"></td>
							<td>
								<form id="form-${schedule.scheduleId}"
									action="/schedule/update/${schedule.scheduleId}" method="post"
							...
							<td>
								<button type="button"
									onclick="deleteSchedule(${schedule.scheduleId})">🗑️</button>
							</td>
						</tr>
					</c:forEach> 	
		...
		document.addEventListener('DOMContentLoaded', function () {
			...
			window.deleteSchedule = function (id) {
				if (confirm("정말 삭제하시겠습니까?")) {
					...
					form.method = 'post';
					form.action = '/schedule/delete/' + id;
